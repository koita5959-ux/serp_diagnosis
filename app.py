...
def results(session_id):
    ...
    for loc_name in location_names:
        ...
        column_count = category_counts.get("コラム", 0)
...
def export_csv(session_id):
    ...
    # メタ情報
    writer.writerow(["検索フレーズ", session["query"] or ""])
    writer.writerow(["実行日時", session["created_at"] or ""])
    writer.writerow(["デバイス", session["device"] or "desktop"])
    writer.writerow([])
    ...
    writer.writerow(["--- 集計 ---"])
    writer.writerow(["広告", len([r for r in all_results if r["is_ad"]])])
    writer.writerow(["ポータル", category_counts.get("ポータル", 0)])
    writer.writerow(["大手", category_counts.get("大手", 0)])
    writer.writerow(["コラム", category_counts.get("コラム", 0)])
    writer.writerow(["その他", category_counts.get("その他", 0)])
    writer.writerow(["企業HP（SEOあり）", category_counts.get("企業HP（SEOあり）", 0)])
    writer.writerow(["企業HP（SEOなし）", category_counts.get("企業HP（SEOなし）", 0)])
    total = len([r for r in all_results if r["is_ad"]]) + sum(category_counts.values())
